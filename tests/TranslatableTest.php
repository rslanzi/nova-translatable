...
use Rslanzi\NovaTranslatable\Translatable;
...
class TranslatableTest extends TestCase
{
    ...
    /** @test */
    public function it_will_can_group_fields_by_locale()
    {
        $translatable = Translatable::make([
            new Text('title'),
        ...
        $this->assertEquals($translatable->data[0]->name, 'Title (es)');
        $this->assertEquals($translatable->data[0]->panel, 'es');
        $this->assertEquals($translatable->data[1]->name, 'Title (fr)');
        $this->assertEquals($translatable->data[1]->panel, 'fr');
    }
